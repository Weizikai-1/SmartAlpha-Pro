...
with st.sidebar:
    st.header("分析参数")
    ticker = st.text_input("股票代码", "000001.SZ", help="Tushare 格式（如 000001.SZ, 600519.SH）")
    trade_date_raw = st.date_input("分析日期", value=datetime.date(2024, 7, 1))
...
GO_STATUS = {"go": ("✅", "success"), "no_go": ("🚫", "error"), "hold": ("⏸️", "warning")}

def _safe_signal(raw: str) -> str:
    return SIGNAL_MAP.get(raw, raw)

# ── Main ──
if analyze_btn:
    try:
        with st.spinner(f"分析中 ({depth} 模式, 约 30s)..."):
        ...
        sig_raw = decision.get("signal", "?")
        sig_cn = SIGNAL_MAP.get(sig_raw, sig_raw)
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            st.metric("决策", f"{SIGNAL_ICON.get(sig_cn, '⬜')} {sig_cn}")
        with c2:
        ...
        if active:
            ...
            for i, (key, name) in enumerate(active):
                ...
                sig_cn = SIGNAL_MAP.get(sig, sig)
                with cols[i]:
                    st.metric(
                        label=f"{SIGNAL_ICON.get(sig_cn, '⬜')} {name}",
                        value=f"{out.get('confidence', 0):.0%}",
                    )
                    with st.expander("详情"):
        ...
        if debate or risk:
            ...
            with col_a:
                st.subheader("多空辩论")
                dsig = _safe_signal(debate.get("signal", "?"))
                st.metric("综合信号", dsig)
                st.write(debate.get("reasoning", ""))
            with col_b:
                st.subheader("风控审核")
                go = risk.get("go_no_go", "?")
                icon, level = GO_STATUS.get(go, ("❓", "info"))
                if level == "success":
                    st.success(f"{icon} 通过")
                elif level == "error":
                    st.error(f"{icon} 不通过 — {go}")
                elif level == "warning":
                    st.warning(f"{icon} 观望 — {go}")
                else:
                    st.info(f"{icon} 状态: {go}")
                st.write(risk.get("risk_reasoning", ""))
                if risk.get("risk_flags"):
                    st.caption("风险标记: " + ", ".join(str(f) for f in risk["risk_flags"]))
        ...
        st.error(f"分析失败: {e}")
        st.caption("请检查股票代码是否正确，或查看终端日志获取详细信息。")
